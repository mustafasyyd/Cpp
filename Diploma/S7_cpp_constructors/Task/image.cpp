...
class image{
    ...
    public: 
    //constructors
    image (const vector <vector <uint16_t>> &image_vec); //parametrized constructor.
    image( const image &img); //copy constructor

    //methods
    void Set_image(const vector <vector <uint16_t>> &image_vec);
    vector <vector <uint16_t>> Get_image ();    
    ...
    private: //attributes
    vector <vector <uint16_t>> image_vec;
};

//definitions 
    //constructors
image::image (const vector <vector <uint16_t>> &image_vec) : image_vec(image_vec) {} //parametrized constructor.

image::image(const image &img) : image_vec (img.image_vec) {} //copy constructor

    //methods
void image::Set_image( const vector <vector <uint16_t>> &image_vec ){
        this->image_vec = image_vec;
}

vector <vector <uint16_t>> image::Get_image (){
    return image_vec;
}

void image::Print_image(){ //range-based loop hat nicht ausgefuert/ging nicht.
...
int main(){
    ...
    //test cases.
    vector <vector <uint16_t>> image_png
    {
        {12, 15, 22},
        {21, 41 , 32}
    };

    vector <vector <uint16_t>> image_jpg
    {
        {144, 132, 234},
        {121, 110 , 136}
    };
